...
def venues():
 ...
 #counts=Venue.query.filter(Venue.city==item.city, Venue.state==item.state).all()
 #for each in counts:
 #  city_info['venues'].append({
 #    "id":each.id,
 #    "name":each.name,
 #    "num_upcoming_shows":len(Venue.query.filter(Show.venue_id==Venue.id, Show.datetime>datetime.now()).all())
 #})
  
  
  # It's for the venue list.....
...
def search_venues():
  search_name=Venue.name.ilike('%{}%'.format(request.form.get('search_term','')))
  search_result=Venue.query.filter(search_name).all()
  response={
  ...
  for item in search_result[:]:
    response['data'].append({
      "id":item.id,
      "name":item.name,
      "num_upcoming_shows":len(Venue.query.filter(Show.venue_id==item.id, Show.datetime>datetime.now()).all())
    })
...
def show_venue(venue_id):
  ...
  # It's used for return to a venue page when click a venue's name......
